...
function setSpeedMode(enabled) {
  ...
  if (btn) {
    btn.textContent = enabled ? "⏩ 倍速模式：开" : "⏩ 倍速模式：关";
    btn.classList.toggle("active", enabled);
  }
  if (enabled) {
    setAIStatus("AI：倍速模式已开启，按需加载");
  } else {
    setAIStatus("AI：准备加载极速模型");
  }
}
...
async function getFreeAI() {
  ...
  if (!freeAIPromise) {
    freeAIPromise = (async () => {
      setAIStatus(speedModeEnabled ? "AI：倍速模式加载中" : "AI：正在加载极速模型，首次会比较慢");
      try {
...
function buildJudgePrompt(question, answer, memoryText) {
  const memoryLine = speedModeEnabled ? "无" : (memoryText || "无");
  return [
    "你是中文海龟汤裁判，回答要极短。",
...
async function judgeWithFreeAI(question, answer, memoryText) {
  ...
  try {
    ...
    const output = await withTimeout(pipe(prompt, {
      max_new_tokens: speedModeEnabled ? 16 : 32,
      do_sample: false,
      temperature: 0.1,
      top_p: 0.8,
      repetition_penalty: 1.05,
      return_full_text: false,
    }), speedModeEnabled ? 850 : FAST_AI_TIMEOUT_MS);
...
async function hintWithFreeAI(score, question, answer, memoryText) {
  ...
  try {
    ...
    const output = await withTimeout(pipe(prompt, {
      max_new_tokens: speedModeEnabled ? 10 : 20,
      do_sample: false,
      temperature: 0.1,
      top_p: 0.8,
      repetition_penalty: 1.05,
      return_full_text: false,
    }), speedModeEnabled ? 650 : FAST_AI_TIMEOUT_MS);
